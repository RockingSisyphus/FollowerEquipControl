// Hooks SkyUI InventoryListEntry.formatName to add Outfit Snapshot Restore icons.
...
// members let this hook coexist with the combat icon injectors through chaining.
	...
	namespace
	{
		...
		[[nodiscard]] bool IsFeatureEnabled()
		{
			...
			return ia.enableIconIndicator &&
			       cfg.enableUpdateNpcOutfitSuppression && cfg.enableOutfitSnapshotRestore && ia.enableOutfitSyncIcon;
		}
		...
		{
			auto entries = OutfitSnapshotRestore::GetSnapshotEntries(a_actorId);
		...
		[[nodiscard]] SnapshotSets CollectSnapshotMatches(RE::ContainerMenu* a_menu)
		{
			...
			auto entries = OutfitSnapshotRestore::GetSnapshotEntries(target->GetFormID());
			if (entries.empty()) {
				return result;
			}
