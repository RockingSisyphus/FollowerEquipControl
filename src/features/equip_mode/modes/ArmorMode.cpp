...
{
	std::optional<Core::StrictPick> ArmorMode::TryBuildStrictPick(
	...
	{
		if (!a_entry || !a_object || a_object->GetFormType() != RE::FormType::Armor) {
			return std::nullopt;
		}
		...
		// Armor is not hand-specific; any worn instance in this row means unequip.
		if (!a_equipOnly) {
